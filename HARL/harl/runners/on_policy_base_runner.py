...
from harl.common.buffers.on_policy_actor_buffer_potential import OnPolicyActorBuffer_Potential
from harl.common.buffers.on_policy_critic_buffer_ep import OnPolicyCriticBufferEP
from harl.common.buffers.on_policy_critic_buffer_ep_potential import OnPolicyCriticBufferEP_Potential
...
class OnPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        if self.share_param:
            self.actor = []
            if self.args["algo"] in self.POTENTIAL_ALGOS:
            ...
            if self.args["algo"] in self.POTENTIAL_ALGOS:
                ...
                for agent_id in range(self.num_agents):
                    agent = ALGO_REGISTRY[args["algo"]]({**algo_args["model"], **algo_args["algo"]}, self.envs.observation_space[0], self.envs.action_space[0], device=self.device,)
                    self.actor.append(agent)
        ...
        if self.algo_args["render"]["use_render"] is False:  # train, not render
            if self.args["algo"] in self.POTENTIAL_ALGOS:
                self.actor_buffer = []
                for agent_id in range(self.num_agents):
                    ac_bu = OnPolicyActorBuffer_Potential(
            ...
            if self.args["algo"] in self.POTENTIAL_ALGOS:
                ...
                self.critic = VCritic_Potential(
                    critic_args, share_observation_space, device=self.device
                )
            ...
            if self.args["algo"] in self.POTENTIAL_ALGOS and self.state_type == "EP":

                self.critic_buffer = OnPolicyCriticBufferEP_Potential(
        ...
        if self.manual_expand_dims:
            ...
            for _ in range(self.algo_args["render"]["render_episodes"]):
                ...
                while True:
                    ...
                    #print("render_actions", eval_actions)
            ...
            # This branch is kept for compatibility with vectorized render envs.
            for _ in range(self.algo_args["render"]["render_episodes"]):
                eval_obs, _, eval_available_actions = self.envs.reset()
                eval_rnn_states = np.zeros(
                    (
