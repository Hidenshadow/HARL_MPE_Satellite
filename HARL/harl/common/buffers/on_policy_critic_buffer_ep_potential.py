...
class OnPolicyCriticBufferEP_Potential:
    """On-policy buffer for critic that uses Environment-Provided (EP) state."""
    ...
    def feed_forward_generator_critic(
        ...
        """
        #print("debug, potential_critic_generator_actor active")
        ...
        #New add-Jianglin
        next_share_obs = self.share_obs[1:].reshape(-1, *self.share_obs.shape[2:])
        ...
        for indices in sampler:
            ...
            #new add Jianglin
            next_share_obs_batch = next_share_obs[indices]
            ...
            yield share_obs_batch, next_share_obs_batch, rnn_states_critic_batch, value_preds_batch, return_batch, masks_batch
