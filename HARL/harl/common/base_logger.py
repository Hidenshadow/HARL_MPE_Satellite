...
class BaseLogger:
    ...
    def __init__(self, args, algo_args, env_args, num_agents, writter, run_dir):
        ...
        #wandb.init(
        #    project=args.get("project_name", "MARL-Training"),  # Set a default project name
        #    name=args.get("exp_name", "experiment"),
        #    config={"algo_args": algo_args, "env_args": env_args},
        #    dir=run_dir
        #)
    ...
    def eval_log(self, eval_episode):
        ...
        # Use the first agent reward from each episode as the global reward.
        if len(self.eval_episode_rewards) > 0:
            global_rewards = [int(ep_rewards[0]) for ep_rewards in self.eval_episode_rewards]
        else:
            global_rewards = []
        ...
        # Write the current total step count and global rewards to the log.
        rewards_str = ",".join(map(str, global_rewards))
