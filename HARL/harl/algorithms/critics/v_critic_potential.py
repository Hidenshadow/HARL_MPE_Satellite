...
class VCritic_Potential():
    ...
    def __init__(self, args, cent_obs_space, device=torch.device("cpu")):
        ...
        self.potential_weight = args.get("potential_weight", 0.1)  # Default weight for Potential Game
    ...
    def update(self, sample, value_normalizer=None):
        ...
        # Compute V(s') for potential-game shaping.
        values_next, _ = self.get_values(next_share_obs_batch, rnn_states_critic_batch, masks_batch)
        adjusted_return = return_batch + self.potential_weight * (values_next - values)

        value_loss = self.cal_value_loss(
            values, value_preds_batch, adjusted_return, value_normalizer=value_normalizer
        )
