"""MAE MAPPO algorithm with intrinsic RND exploration bonus."""
import numpy as np
import torch
...
class MAE_MAPPO(OnPolicyBase):
    def __init__(self, args, obs_space, act_space, num_agents, device=torch.device("cpu")):
        super(MAE_MAPPO, self).__init__(args, obs_space, act_space, device)
    ...
    def update(self, sample):
        ...
        centralized_obs = torch.cat([obs_flat] * agent_num, dim=-1).to(self.device)

        if next_obs_batch is not None:
            next_flat = next_obs_batch.reshape(batch_size, -1)
            centralized_next_obs = torch.cat([next_flat] * agent_num, dim=-1).to(self.device)
        else:
        ...
        for i in range(agent_num):
            m = centralized_obs.clone()
            start = i * local_dim
            m[:, start:start+local_dim] = 0
            mc, _ = self.critic.critic(m, rnn_states_batch, masks_batch)
        ...
        # --- RND ---
        with torch.no_grad():
            tgt_feat = self.rnd_target(centralized_obs)
        pred_feat = self.rnd_predictor(centralized_obs)
