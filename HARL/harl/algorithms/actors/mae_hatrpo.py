...
class MAE_HATRPO(OnPolicyBase):
    def __init__(self, args, obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        super(MAE_HATRPO, self).__init__(args, obs_space, act_space, device)
        ...
        # Potential-game shaping parameters.
        self.potential_weight = args.get("potential_weight", 0.25)  # potential-game weight
        self.coop_bonus_weight = args.get("coop_bonus_weight", 0.15)  # cooperative bonus weight
        self.int_rnd_weight = args.get("int_rnd_weight", 0.1)  # RND intrinsic reward weight
    ...
    def update(self, sample):
        ...
        # Handle observation batches with different ranks.
        if obs_batch.ndim == 4:  # 4D: (batch_size, num_agents, obs_dim, 1)
            obs_flat = obs_batch.reshape(-1, obs_batch.shape[-2])
            next_obs_flat = obs_batch[:, 1:].reshape(-1, obs_batch.shape[-2])
            ...
            next_obs_flat = obs_batch[:, 1:].reshape(-1, obs_batch.shape[-1])
            rnn_states_batch_flat = rnn_states_batch.reshape(-1, *rnn_states_batch.shape[2:])
            masks_batch_flat = masks_batch.reshape(-1, *masks_batch.shape[2:])
        else:  # 2D: (batch_size, obs_dim)
            obs_flat = obs_batch
            next_obs_flat = obs_batch
