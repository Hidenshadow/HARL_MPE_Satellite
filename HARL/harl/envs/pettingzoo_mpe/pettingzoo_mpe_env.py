...
class PettingZooMPEEnv:
    def __init__(self, args):
        ...
        if self.scenario != "simple_satellite":
            self.args.pop("map_name", None)
        ...
        if "max_cycles" not in self.args or self.args["max_cycles"] is None:
            self.args["max_cycles"] = self._default_max_cycles()
    ...
    def _default_max_cycles(self):
        if self.scenario == "simple_satellite":
            map_name = self.args.get(
                "map_name", getattr(self.module, "DEFAULT_MAP_NAME", "2x2-4A")
            )
            return self.module.get_satellite_config(map_name)["episode_length"]
        return 100
