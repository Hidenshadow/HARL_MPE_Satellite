...
def update_cosine_schedule(
    ...
    if epoch < warmup_epochs:
        learning_rate = initial_lr * epoch / max(1, warmup_epochs)
    else:
        progress = (epoch - warmup_epochs) / max(1, total_num_epochs - warmup_epochs)
        learning_rate = min_lr + 0.5 * (initial_lr - min_lr) * (
