...
def get_task_name(env, env_args):
    ...
    if env == "pettingzoo_mpe":
        action_type = "continuous" if env_args["continuous_actions"] else "discrete"
        if env_args.get("scenario") == "simple_satellite" and "map_name" in env_args:
            task = f"{env_args['scenario']}-{env_args['map_name']}-{action_type}"
        else:
            task = f"{env_args['scenario']}-{action_type}"
    else:
