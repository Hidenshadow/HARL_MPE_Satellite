...
def _reset_aec_env(aec_env, seed=None, return_info=False, options=None):
    try:
        return aec_env.reset(seed=seed, return_info=return_info, options=options)
    except TypeError as exc:
        if "return_info" not in str(exc):
            raise
        return aec_env.reset(seed=seed, options=options)
...
class aec_to_parallel_wrapper(ParallelEnv):
    ...
    def reset(self, seed=None, return_info=False, options=None):
        _reset_aec_env(
            self.aec_env, seed=seed, return_info=return_info, options=options
        )
